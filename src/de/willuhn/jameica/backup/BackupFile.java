...
public class BackupFile implements GenericObject
{
  ...
  private void load() throws ApplicationException
  {
    ...
    try(ZipFile zip = new ZipFile(this.file))
    {
      ...
      if (entry == null)
        throw new ApplicationException(Application.getI18n().tr("Backup enth�lt keine Plugin-Daten"));
      
      try (InputStream is = zip.getInputStream(entry)) {
        if (is == null)
          throw new ApplicationException(Application.getI18n().tr("Ung�ltiges Backup-Format"));
