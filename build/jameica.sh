...
bit=`uname -m |grep 64`
if [ $? = 0 ]
 then LIBOVERLAY_SCROLLBAR=0 GDK_NATIVE_WINDOWS=1 java -Xmx256m $_JCONSOLE -jar jameica-linux64.jar $@
 else LIBOVERLAY_SCROLLBAR=0 GDK_NATIVE_WINDOWS=1 java -Xmx256m $_JCONSOLE -jar jameica-linux.jar $@
fi
